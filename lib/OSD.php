...
class OSD {
    ...
    public static function setup($client_id, $client_secret, $options = ['session_manager' => 'OSDSession', 'curl_options' => []]) {
        // Setup client info
    ...
    public static function authenticate_with_authorization_code($authorization_code) {
        $data = [];
        $auth_type = ['type' => 'authorization_code'];

        $data['grant_type'] = 'authorization_code';
        $data['code'] = $authorization_code;

        $request_data = array_merge($data, ['client_id' => self::$client_id]);
        if ($response = self::request(self::GET, '/oauth/access_token', $request_data, ['oauth_request' => true])) {
    ...
    public static function authenticate_with_password($user_id, $password) {
        ...
        $response = self::request(self::POST, '/oauth/v2/token', $attributes, ['oauth_request' => true]);
    ...
    public static function request($method, $url, $attributes = [], $options = []) {
        ...
        switch ($method) {
            ...
            case self::POST:
                ...
                } else if (empty($options['oauth_request'])) {
                    // application/json
        ...
        switch ($response->status) {
            ...
            case 401 :
                ...
                } else if (strstr($body['error'], 'invalid_request')) {
                    // print_object($body);
